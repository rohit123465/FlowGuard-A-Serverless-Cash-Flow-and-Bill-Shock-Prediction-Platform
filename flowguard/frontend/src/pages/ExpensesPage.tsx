  ...
  return (
    <>
      ...
      <section className="panel">
        ...
        <ExpenseList expenses={query.data ?? []} loading={query.isLoading} onEdit={(expense) => { setEditing(expense); setFormOpen(true); }} onDelete={(expense) => { if (window.confirm(`Delete “${expense.description}”?`)) remove.mutate(expense); }} renderReceipt={(expense) => <ReceiptUploader expense={expense} onChanged={() => queryClient.invalidateQueries({ queryKey: ["expenses"] })} onApplied={async (updated) => { const updatedMonth = updated.expense_date.slice(0, 7); setMonth(updatedMonth); await queryClient.invalidateQueries({ queryKey: ["expenses"] }); const label = new Date(`${updatedMonth}-01T00:00:00`).toLocaleDateString("en-GB", { month: "long", year: "numeric" }); setNotice(`Receipt details applied. Showing ${label}, where the expense is now recorded.`); }} />} />
      </section>
