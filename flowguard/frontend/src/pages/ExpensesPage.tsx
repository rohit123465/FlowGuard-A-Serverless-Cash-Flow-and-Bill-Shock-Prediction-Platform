...
export function ExpensesPage() {
  ...
  return (
    <>
      <div className="page-heading"><div><p className="eyebrow">Spending ledger</p><h1>Expenses</h1><p className="muted">Track cleared and planned spending in one secure place.</p></div><button className="button button-primary" onClick={() => { setEditing(null); setFormOpen(true); }}>+ Add expense</button></div>
      ...
      <section className="panel">
        ...
        <ExpenseList expenses={query.data ?? []} loading={query.isLoading} onEdit={(expense) => { setEditing(expense); setFormOpen(true); }} onDelete={(expense) => { if (window.confirm(`Delete “${expense.description}”?`)) remove.mutate(expense); }} />
      </section>
